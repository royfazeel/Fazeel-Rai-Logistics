...
## Homepage image release: final SEO review

Reviewed on 24 September 2026 while preparing the approved homepage image and slogan for production. The slogan is **“You drive the miles. We manage the loads.”** The search title remains service-specific: **“Truck Dispatch Services USA | 5–8% Fees | Rai Dispatch.”** The description explains the audience, coverage, included work and equipment-based fees; the social sharing graphic uses the approved slogan. The homepage Service schema specifies the **48 contiguous United States**, matching the actual service boundary. The new image at `/images/rai-dispatch-hero-realistic.png` is included in the homepage sitemap entry and identified as the homepage's primary image in WebPage structured data; it is an illustrative generated brand image, not evidence of a company-owned carrier fleet.

The read-only production crawl before this release completed at **17:02:49 UTC on 24 September 2026**: **40 of 40 sitemap pages**, **212 distinct internal link/fragment targets**, **0 failures**, and **0 advisory warnings**. It checked server-rendered titles, descriptions, H1s, canonical and Open Graph URLs, indexability, JSON-LD syntax, internal destinations and the approved fee associations. This is a baseline for the final release; repeat the same check after deployment and record the resulting commit and URL. It does not establish Google indexing, a rich result, rankings, or real-user performance.

Existing content already covers distinct dispatch intents through **8 equipment pages, 9 service pages, 3 carrier pages and 7 practical guides**, in addition to the home, category, pricing, coverage and company pages. Equipment coverage includes cargo/Sprinter vans, box trucks, hotshot, dry van, flatbed, reefer, power only and step deck. Service coverage includes load booking, rates, broker communication, lane planning, paperwork, scheduling, regional, OTR and dedicated dispatch. Carrier pages address owner-operators, small fleets and new authorities. The “other equipment” 7% category remains explained in pricing and requires a fit review; a new page is appropriate only when its service details can be described accurately.

Current Google guidance was rechecked for this release: use [clear page titles, useful descriptions and natural language](https://developers.google.com/search/docs/fundamentals/seo-starter-guide), [canonical sitemap URLs and accurate modification dates](https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap), and [structured data that matches visible content](https://developers.google.com/search/docs/appearance/structured-data/sd-policies). For the replacement photograph, use a descriptive image filename and alt text, responsive image sizes and a crawlable HTML image; [Google does not index CSS background images](https://developers.google.com/search/docs/appearance/google-images). Do not add meta-keyword lists, copied location pages or unsupported rating/market-leadership claims.
...
### Photographic homepage release — 24 September 2026

The approved homepage replaces the hero video with an original AI-generated branded truck image. Desktop uses an uncropped wide composition and a narrower text column; phone layouts put the selected slogan and complete truck above the description and actions. The slogan is “You drive the miles. We manage the loads.” The image is a promotional brand concept, not evidence of a company-owned fleet. Next.js serves responsive optimized versions of the 1916×821 master.

The existing production analytics, lead delivery, domain redirects, support email, and equipment rates are preserved. Both lead forms now include Other truck type, whose equipment fee is 7%.

Pre-deployment validation: production build generated all 46 outputs; local production crawl passed 40/40 pages and 212 internal targets with no failures or warnings. All 17 isolated lead API checks passed using only a local test provider. The setup modal and truck imagery were checked at normal desktop and 375px phone width; 320px had no horizontal overflow. Social graphic and hero master returned HTTP 200 and valid PNGs. No production test emails were sent. These checks do not establish inbox delivery, search rankings, or physical-device performance.
