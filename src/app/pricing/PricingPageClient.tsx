...
const pricingFaqs = [
  ...
  { id: 7, question: 'Is a dedicated truck dispatcher included?', answer: 'Yes. Clients receive a dedicated truck dispatcher for their agreed load search, rate negotiation, booking coordination and broker communication. Confirm your contact person, desk hours and escalation process during onboarding.' },
];
    ...
    <section className="section-padding bg-surface-50">
      <div className="container-custom grid lg:grid-cols-2 gap-10">
        <div>
          ...
          <h2 className="section-heading mb-5">Calculate the fee for your truck.</h2>
          <p className="text-surface-700 text-lg leading-relaxed mb-5">Multiply the agreed gross load revenue by your equipment percentage. These examples use a $2,000 billing base; the balance is before fuel, insurance, truck payments, factoring, taxes and other expenses.</p>
          <div className="overflow-x-auto rounded-lg border border-surface-300 bg-white mb-5">
          ...
          <Link href="/resources/truck-dispatch-fees" className="inline-flex items-center gap-2 text-primary-700 font-semibold py-2">Read the dispatch fee guide<ArrowRight className="w-4 h-4" aria-hidden="true" /></Link>
        </div>
        <div className="card p-7"><h3 className="font-display text-3xl font-bold text-navy-950 mb-5">Confirm these details in writing.</h3><ul className="space-y-4">{['Your equipment percentage and what revenue it applies to.', 'Your dedicated dispatcher, included services and operating lanes.', 'Invoice timing and payment arrangements.', 'How accessorial payments and cancelled loads are treated.', 'Notice requirements and any outstanding balances.'].map(item => <li key={item} className="flex gap-3 text-surface-700"><CheckCircle className="w-5 h-5 shrink-0 text-primary-600 mt-0.5" aria-hidden="true" />{item}</li>)}</ul></div>
      </div>
    </section>
